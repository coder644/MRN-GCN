...
from train.ari.model.nestEGCN import nestGCN,nestEGCN,nestEGCNs
...
trainset = NestDataset(file_graph_path='../../DGLData/raw_data/nest_new/file_ari_nest.json',
                       func_graph_path="../../DGLData/raw_data/nest_new/func_ari_nest.json",
                       save_dir="../../DGLData/cache/reen_cd.bin",
                       name='reen', force_reload=True, ratio=0.8
                       )
testset = NestDataset(file_graph_path='../../DGLData/raw_data/nest_new/file_ari_nest.json',
                      func_graph_path="../../DGLData/raw_data/nest_new/func_ari_nest.json",
...
for round in range(rounds):
    ...
    if MODEL == 'nestEGCN':
        if sa == False:
            model = nestEGCN().cuda().train() #MRNGCN w/o SA
        else:
            model = nestEGCNs().cuda().train() #MRNGCN

    if MODEL == 'nestGCN':
        model = nestGCN().cuda().train()#N-GCN

    opt = torch.optim.SGD(model.parameters(),lr=0.0002,momentum=0.0005)
    ...
    for epoch in range(EPOCH):
        ...
        with torch.no_grad():
            ...
            print(bads)
            # print('result:'+ str(result[0]) + ', '+ str(result[1]) + ', '+ str(result[2]) + ', '+ str(result[3]) + ', \n' )
