...
class nestGCN(nn.Module):
    def __init__(self):
        ...
        self.gconvs = []
        self.gconvs.append(
            dglnn.GATConv(emb_dim, hidden_dim, num_heads=1, allow_zero_in_degree=True, bias=True).cuda())
...
class nestEGCN(nn.Module):
    def __init__(self):
        ...
        self.gconvs = []
        self.gconvs.append(
            dglnn.EGATConv(emb_dim, hidden_dim, num_heads=num_heads, allow_zero_in_degree=True, bias=True).cuda())
        for i in range(num_layers - 1):
            self.gconvs.append(
                dglnn.EGATConv(hidden_dim * num_heads, hidden_dim, num_heads=num_heads, allow_zero_in_degree=True,
                               bias=True).cuda())
