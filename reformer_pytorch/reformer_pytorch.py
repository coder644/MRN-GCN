...
class LSHSelfAttention(nn.Module):
    ...
    def forward(self, x, keys = None, input_mask = None, input_attn_mask = None, context_mask = None, **kwargs):
        ...
        masks = {}                  #wo zi ji zhu shi diao le
...
class ReformerLM(nn.Module):
    def __init__(self, num_tokens, dim, depth, max_seq_len, filters,heads = 8, dim_head = None, bucket_size = 64, n_hashes = 4, ff_chunks = 100, attn_chunks = 1, causal = False, weight_tie = False, lsh_dropout = 0., ff_dropout = 0., ff_mult = 4, ff_activation = None, ff_glu = False, post_attn_dropout = 0., layer_dropout = 0., random_rotations_per_head = False, twin_attention = False, use_scale_norm = False, use_rezero = False, use_full_attn = False, full_attn_thres = 0, reverse_thres = 0, num_mem_kv = 0, one_value_head = False, emb_dim = None, return_embeddings = False, weight_tie_embedding = False, fixed_position_emb = False, absolute_position_emb = True, axial_position_shape = False, n_local_attn_heads = 0, pkm_layers = tuple(), pkm_num_keys = 128):
        super().__init__()
        ...
        if return_embeddings:
            self.out = Identity()
            return
        #维度[seq_length, embedding_size]
        con1 = nn.Conv2d(in_channels=1, out_channels=filters, kernel_size=(1, dim), bias=True)
        # con2 = nn.Conv2d(in_channels=filters,out_channels= emb_dim,kernel_size=1,bias=True)
        # torch.nn.init.xavier_normal(con1.weight,-1)
        torch.nn.init.xavier_normal_(con1.weight,1)
        # torch.nn.init.xavier_normal(con2.weight,1)
        ...
        linear = nn.Linear(self.max_seq_len*(filters),2)
        torch.nn.init.xavier_normal_(linear.weight,1)
    ...
    def forward(self, x, **kwargs):
        ...
        x = x.view(-1,1,self.max_seq_len,self.dim)

        x = self.conv(x)
        x = x.view(-1,self.max_seq_len * self.filters)
        x = self.classifier(x)
        return x
