...
def generate_data(global_graph_path,f_v_path,f_no_v_path,global_save_path,f_save_path):
                ...
                for node in graph.keys():
                    ...
                    if (graph[node] != []):
                        for edge in graph[node]:
                            ...
                            dst.append(edge.dst.index)
                            e_attr.append(edge.token)
                assert(len(n_attr) == len(graph.keys()))
                assert(len(e_attr) == len(src))
                graph_data = [src, dst, n_attr, e_attr]
                if (len(src) == 0):
                    graph = False
                    print('----------------------------------')
                    break
                # add func in temp list
...
if __name__ == '__main__':
    ...
    # global_save_path = "../raw_data/nest/file_reen_nest.json"
    # f_save_path = "../raw_data/nest/func_reen_nest.json"
    ...
    global_save_path = "../raw_data/nest/file_time_nest.json"
    f_save_path = "../raw_data/nest/func_time_nest.json"
    ...
    generate_data(global_graph_path, v_f_path, no_v_f_path, global_save_path, f_save_path)